...
/**
 * CinsayAdmin related service bus calls.
 */

/**
 * Retrieve a list of an accounts avaialable channel partners, including nested 
 * children.
 *
 * @return string
 */
function ServicebusGetChannelPartners()
{
    $path = '/api/channelPartner/get';
    $request = array(
        'apiKey' => '{{publicKey}}',
    ...
    return ServicebusCachableRequest('channel partners', $path, json_encode($request));
}


//// Authentication calls. We never want to cache
function ServicebusLogin($email, $password) {
    $request = '{"apiKey":"{{publicKey}}","requestObject":{"email":"{{email}}","password":"{{password}}","typeName":"clientAuth"},"params":null}';
